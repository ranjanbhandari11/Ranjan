...
const SUPABASE_DELETIONS_TABLE = process.env.SUPABASE_DELETIONS_TABLE || "account_deletions";
...
let supabase = null;
const unsupportedSupabaseColumns = new Set();
...
function getMissingSupabaseColumn(error) {
  ...
  if (schemaCacheMatch?.[1]) {
    return normalizeTerm(schemaCacheMatch[1]);
  }

  const missingColumnMatch = message.match(/column ['"]?([a-z0-9_]+)['"]? does not exist/i);
  if (missingColumnMatch?.[1]) {
    return normalizeTerm(missingColumnMatch[1]);
  }
...
function buildSupabasePayload(payload) {
  ...
  unsupportedSupabaseColumns.forEach((column) => {
...
async function fallbackToFileStorage(error) {
  ...
  storageMode = "file";
  lastStorageFallbackReason = String(error?.message || error || "unknown fallback reason");
...
function publicEmailError(error, provider = "email") {
  const raw = String(error?.response || error?.message || error || "").trim();
  if (!raw) return `${provider}-failed`;
  return raw
    .replace(SMTP_PASS, "[smtp-password]")
    .replace(SMTP_USER, "[smtp-user]")
    .slice(0, 240);
}
...
async function sendEmailDetailed({ to, subject, text, html }) {
  ...
  if (mailer) {
    try {
      ...
      return { sent: true, provider: "smtp", messageId: info?.messageId || "", attempts };
    } catch (error) {
      const reason = publicEmailError(error, "smtp");
      attempts.push({ provider: "smtp", reason });
      console.error("PantryPal SMTP email delivery failed:", reason);
    }
  } else {
    attempts.push({ provider: "smtp", reason: "smtp-not-configured" });
  }
  ...
  return {
    sent: false,
    provider: null,
    reason: attempts.map((attempt) => `${attempt.provider}: ${attempt.reason}`).join(" | ") || "email-delivery-failed",
    attempts
  };
}
...
async function fetchJsonWithTimeout(url, options, label) {
  ...
  }, EXTERNAL_REQUEST_TIMEOUT_MS, label);
}
...
async function sendGoogleLoginNotification(user) {
  ...
  if (!delivery.sent) {
    return { sent: false, reason: delivery.reason || "mailer-unavailable", delivery };
  }
  ...
  return { sent: true, user: updatedUser, provider: delivery.provider, messageId: delivery.messageId || "" };
}
...
function getCalendarDaysLeft(endsAt) {
  if (!endsAt) {
    return 0;
  }
  ...
  return Math.max(0, Math.ceil((endDay.getTime() - todayStart.getTime()) / 86400000));
}
...
async function runSubscriptionNotifications() {
  ...
  for (const rawUser of users) {
    ...
    const now = new Date();
    const daysLeft = getCalendarDaysLeft(subscription.endsAt);
...
async function ensureFileStore() {
  ...
  try {
    await fs.access(ACCOUNT_DELETIONS_FILE);
  } catch {
    await fs.writeFile(ACCOUNT_DELETIONS_FILE, "[]", "utf8");
  }
}
...
async function upsertUser(user) {
  ...
  if (storageMode === "supabase") {
    let nextPayload = buildSupabasePayload(payload);
    ...
    for (let attempt = 0; attempt < 12; attempt += 1) {
      try {
        ...
        if (missingColumn && Object.prototype.hasOwnProperty.call(nextPayload, missingColumn)) {
          unsupportedSupabaseColumns.add(missingColumn);
          delete nextPayload[missingColumn];
          console.warn(
            `Supabase column '${missingColumn}' is not available in '${SUPABASE_USERS_TABLE}'. Retrying without it.`
          );
          continue;
        }
...
async function recordAccountDeletion(user, reason, req) {
  const deletionRecord = {
    ...
    source: isHostedRuntime() ? "hosted" : "local",
  ...
  await ensureFileStore();
  const raw = await fs.readFile(ACCOUNT_DELETIONS_FILE, "utf8");
  const records = JSON.parse(raw);
  records.push(deletionRecord);
  await fs.writeFile(ACCOUNT_DELETIONS_FILE, JSON.stringify(records, null, 2), "utf8");
  return deletionRecord;
}
...
function getFirebaseSafePassword(user) {
  const existingPassword = String(user?.password || "");
  if (existingPassword.length >= 6) {
    return existingPassword;
  }

  const seed = `${user?.email || user?.username || "pantrypal"}:${process.env.FIREBASE_API_KEY || "firebase"}`;
  return `PantryPal-${crypto.createHash("sha256").update(seed).digest("hex").slice(0, 24)}!`;
}
  ...
  try {
    const { response, payload } = await fetchJsonWithTimeout(
      `${FIREBASE_IDENTITY_TOOLKIT_BASE}/accounts:signUp?key=${encodeURIComponent(FIREBASE_PUBLIC_CONFIG.apiKey)}`,
      {
        ...
        body: JSON.stringify({
          email: user.email,
          password: getFirebaseSafePassword(user),
...
async function sendFirebaseEmailVerification(user) {
  ...
  try {
    const { response: signInResponse, payload: signInPayload } = await fetchJsonWithTimeout(
      `${FIREBASE_IDENTITY_TOOLKIT_BASE}/accounts:signInWithPassword?key=${encodeURIComponent(FIREBASE_PUBLIC_CONFIG.apiKey)}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    const { response, payload } = await fetchJsonWithTimeout(
      `${FIREBASE_IDENTITY_TOOLKIT_BASE}/accounts:sendOobCode?key=${encodeURIComponent(FIREBASE_PUBLIC_CONFIG.apiKey)}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
...
async function sendFirebasePasswordResetEmail(user) {
  if (!FIREBASE_PUBLIC_CONFIG.apiKey) {
    return { sent: false, reason: "firebase-not-configured" };
  }
  if (!user?.email) {
  ...
  try {
    ...
    return {
      sent: false,
      reason: payload?.error?.message || "firebase-reset-email-failed",
      payload
    };
  } catch (error) {
...
app.get("/api/health", async (_req, res) => {
  try {
    ...
    res.json({
      ...
      fallbackReason: storageMode === "file" ? lastStorageFallbackReason || null : null,
      unsupportedSupabaseColumns: [...unsupportedSupabaseColumns],
      runtime: {
...
app.get("/api/public-config", (req, res) => {
  ...
  res.json({
    app: {
      ...
      passwordResetEmailEnabled: Boolean((mailer && EMAIL_FROM) || RESEND_API_KEY || FIREBASE_PUBLIC_CONFIG.apiKey)
    },
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (String(user.accountStatus || "active").toLowerCase() !== "active") {
      res.status(403).json({
        error: "This PantryPal account is not currently available for sign-in."
      });
      return;
    }
...
app.post("/api/auth/request-password-reset", async (req, res) => {
  try {
    ...
    if ((!mailer || !EMAIL_FROM) && !RESEND_API_KEY && !FIREBASE_PUBLIC_CONFIG.apiKey) {
      res.status(503).json({
        error: "Password reset email is not configured yet. Add SMTP, Resend, or Firebase settings to PantryPal first."
      });
      return;
    }
    ...
    if (!notificationEmailSent) {
      ...
      const firebaseReason = firebaseReset.reason || "firebase-not-sent";
      res.status(502).json({
        error: `PantryPal could not send the password reset email. Email provider error: ${smtpReason}. Firebase fallback: ${firebaseReason}.`,
        ok: false,
        email: user.email,
        username: user.username,
        resetLinkIssued: true,
        notificationEmailSent,
        emailDeliveryReason: `${smtpReason} | ${firebaseReason}`,
    ...
    res.status(201).json({
      ...
      emailDeliveryDetails: {
        pantrypalEmail: {
          ...
          provider: signupNotice.provider || null,
          reason: signupNotice.reason || ""
        },
        firebase: {
          sent: Boolean(firebaseVerification.sent),
          reason: firebaseVerification.reason || ""
        }
      },
      firebaseBootstrap: {
...
app.delete("/api/users/:email", async (req, res) => {
  try {
    ...
    if (reason.length < 8) {
      res.status(400).json({ error: "Please provide a reason before deleting this account." });
      return;
    }

    await recordAccountDeletion(existingUser, reason, req);
    await sendEmail({
...
app.post("/create-checkout-session", async (req, res) => {
  try {
    ...
    const origin = getCanonicalAppOrigin(req);
    const planText = String(plan.name).toLowerCase();
    const isWeekly = planText.includes("weekly") || planText.includes("7");
