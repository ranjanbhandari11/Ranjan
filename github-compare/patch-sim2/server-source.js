...
async function sendEmail({ to, subject, text, html }) {
  ...
  try {
    ...
    console.error("PantryPal email delivery failed:", error.message);
    if (normalizeTerm(SMTP_HOST) === "smtp.gmail.com" && SMTP_USER && SMTP_PASS) {
      try {
        ...
        await gmailFallbackMailer.sendMail({
          from: EMAIL_FROM,
          to,
          subject,
          text,
        ...
        console.error("PantryPal Gmail fallback delivery failed:", fallbackError.message);
      }
    }
    return false;
  }
...
app.get("/api/public-config", (req, res) => {
  ...
  res.json({
    app: {
      ...
      passwordResetEmailEnabled: Boolean((mailer && EMAIL_FROM) || FIREBASE_PUBLIC_CONFIG.apiKey)
    },
...
app.post("/api/auth/request-password-reset", async (req, res) => {
  try {
    ...
    if ((!mailer || !EMAIL_FROM) && !FIREBASE_PUBLIC_CONFIG.apiKey) {
      res.status(503).json({
        error: "Password reset email is not configured yet. Add SMTP or Firebase settings to PantryPal first."
      });
      return;
    }
    ...
    res.json({
      ...
      email: user.email,
      username: user.username,
      delivery: resetNotice.sent ? "pantrypal" : (firebaseReset.sent ? "firebase" : "none"),
      resetLinkIssued: true,
      notificationEmailSent,
      emailDeliveryReason: notificationEmailSent ? "sent" : (firebaseReset.reason || resetNotice.reason || "email-delivery-failed"),
      expiresAt: resetExpiresAt
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  ...
  try {
    ...
    res.status(201).json({
      ...
      emailDeliveryReason: notificationEmailSent
        ? (signupNotice.sent ? "smtp-sent" : "firebase-verification-sent")
